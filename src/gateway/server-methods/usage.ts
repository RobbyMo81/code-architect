...
import { upsertRuntimeUsage } from "../usage-runtime-db.js";
...
export const usageHandlers: GatewayRequestHandlers = {
  ...
  "usage.runtime": async ({ respond, params }) => {
    const key = typeof params?.sessionKey === "string" ? params.sessionKey.trim() : "";
    if (!key) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "usage.runtime requires sessionKey"),
      );
      return;
    }
    ...
    const usageModel = usage?.modelUsage?.[0];
    const provider = resolved.entry?.modelProvider ?? usageModel?.provider ?? null;
    const model = resolved.entry?.modelOverride ?? usageModel?.model ?? null;

    const snapshot = upsertRuntimeUsage({
      sessionKey: key,
